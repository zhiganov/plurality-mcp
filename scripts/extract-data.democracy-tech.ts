...
- **Glossary:** a *book-coined or book-emphasized* term, named explicitly as a defined concept — italicized at first use, set in quotation marks, introduced via a definition sentence, OR appearing in the book's own glossary section. Common English phrases the book uses but doesn't formally define DO NOT belong here. The \`term\` field MUST be the term EXACTLY AS THE BOOK USES IT — NO parenthetical disambiguator like "(as ⿻ X)", "(⿻ Y)", "(Taiwan)", "(Collaboration)", "(Failure Mode)", "(Communication Layer)", etc. The model adds these as chapter/category anchors; they are NOT part of the term and they defeat the semantic dedup pass. Romanization or original-language glyph in parens IS allowed when the book uses both forms ("Dao (道)", "Yushan View (玉山觀點)"). BEFORE emitting to glossary, ROUTE CROSS-CATALOG CANDIDATES: a specific named case (vTaiwan, Polis, g0v, Decidim) → \`cases\`; a counter-pattern (monist-technocracy, atomist-libertarianism, authoritarian digital governance) → \`failure_modes\`; a scaled policy/regulatory/movement move (interoperability mandates, public-option platforms, data-union law) → \`policy_strategies\`; a named governance primitive (quadratic voting, augmented deliberation, polis-clustering) → \`governance_forms\`; a verbatim quote-shaped paragraph → \`quotes\`. Only emit to \`glossary\` if the candidate is none of those — a vocabulary term with no narrower home. Examples of genuine glossary entries: "⿻", "post-symbolic communication", "augmented deliberation" (as a primitive — but if used as a generic concept, prefer governance_forms), "intersectional social identity", "plurality publics", "rights os", "monist", "atomist", "lost dao", "yushan view". Examples that DO NOT belong: "polarization" (common English), "Wikipedia" (a case), "Plurality Institute" (a case), "Polis" (a case), "Interoperability Mandates" (a policy_strategy).
...
- glossary: ~40 distinct terms (HARD GUIDANCE — if you find yourself approaching 80+, you are over-extracting. Check the cross-catalog routing rule above and discard candidates that belong in cases / failure_modes / policy_strategies / governance_forms / quotes. The glossary is for terms with NO narrower home, not a vacuum for everything interesting.)
